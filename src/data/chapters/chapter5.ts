  ...
  sections: [
    {
      ...
      problemCount: 3,
      blocks: [
        { kind: 'prose', markdown: `### Definition and Structure
...
A **transition graph** depicts states as nodes and transition probabilities as directed edges, making possible and impossible transitions immediately visible.` },
        { kind: 'prose', markdown: `### Classification of States
...
- **Absorbing state:** State i with pᵢᵢ = 1 (impossible to leave). A **Markov chain is absorbing** if it has at least one absorbing state and every state can reach an absorbing state.` },
        { kind: 'prose', markdown: `### Key Equations for Absorbing Chains
...
(The "+1" accounts for the one step taken to reach the next state.)` },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-1-01-gambler-s-ruin-markov-chain-approach",
            chapter: 5,
            section: "5.1",
            sectionTitle: "Markov Chain",
            title: "Gambler's Ruin (Markov Chain Approach)",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `Player M has $1 and player N has $2. Each game, the winner takes $1 from the other. M wins 2/3 of the time. They play until one is bankrupt. What is the probability M wins?`,
            solution: `State space: {m} = {0, 1, 2, 3} (M's current balance), where 0 and 3 are absorbing states.
...
**P(M wins) = 4/7 ≈ 0.571.** Starting from $2, P(M wins) = 6/7.`,
        ...
        },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-1-02-dice-question-12-vs-7-7",
            chapter: 5,
            section: "5.1",
            sectionTitle: "Markov Chain",
            title: "Dice Question (12 vs. 7-7)",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `Player A bets that a sum of 12 will appear before two consecutive 7s (Player B). What is the probability A wins?`,
            solution: `Probabilities per roll: \`P(12) = 1/36\`, \`P(7) = 6/36\`, \`P(other) = 29/36\`.
...
Both methods yield: **P(A wins) = 7/13**`,
        ...
        { kind: 'prose', markdown: `**Coin Triplets**
...
**Summary: E[HHH] = 14 tosses, E[THH] = 8 tosses.**` },
        { kind: 'prose', markdown: `**Part B — Probability of HHH before THH**
...
Setting up states S, T, TH, H, HH, HHH, THH, and solving for probability of reaching HHH from S, we get aₛ = **1/8**.` },
        { kind: 'prose', markdown: `**Part C — Choosing Sequences (Penney's Game)**
...
> **Conclusion:** Player 1 should always go **second**. No matter what sequence player 1 picks, player 2 can always achieve at least 2/3 probability of winning. **Never go first.**` },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-1-03-markov-chain-problem-3",
            chapter: 5,
            section: "5.1",
            sectionTitle: "Markov Chain",
            title: "Markov Chain Problem 3",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `A box contains n balls of n different colors. Each step: randomly select a pair (ordered), repaint the first to match the second, return both. What is the expected number of steps until all balls are the same color?`,
...
---`,
    ...
    },
    {
      ...
      problemCount: 4,
      blocks: [
        { kind: 'prose', markdown: `### Definitions
...
The conditional expected future value equals the current value. A martingale is NOT necessarily a Markov process (and vice versa).` },
        { kind: 'prose', markdown: `### Stopping Rules and Wald's Equality
...
*Proof sketch:* Write Sₙ = Σₙ Xₙ Iₙ where Iₙ = 1 if N ≥ n, 0 otherwise. Since Iₙ is independent of Xₙ (stopping depends only on past): \`E[XₙIₙ] = E[X]E[Iₙ]\`. Summing: \`E[Sₙ] = E[X]×Σₙ E[Iₙ] = E[X]×E[N]\`.` },
        { kind: 'prose', markdown: `### General Martingale Results for Random Walk
...
\`\`\`` },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-2-01-drunk-man-on-bridge",
            chapter: 5,
            section: "5.2",
            sectionTitle: "Martingale and Random Walk",
            title: "Drunk Man on Bridge",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `A drunk man stands at position 17 on a 100-meter bridge (positions 0 to 100). He steps forward or backward with equal probability 1/2. What is the probability he reaches the end (position 100) before the beginning (position 0)? What is the expected number of steps?`,
...
- From E[Sₙ²−N] = 0: \`p_α × 83² + (1−p_α) × 17² = E[N]  →  E[N] = (17×83²+83×17²)/100 = 17×83 = 1411\` ✓`,
        ...
        },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-2-02-dice-game-wald-s-equality",
            chapter: 5,
            section: "5.2",
            sectionTitle: "Martingale and Random Walk",
            ...
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `Roll a die; receive the face value. On a 4, 5, or 6 you may roll again. Game stops on 1, 2, or 3. Expected total payoff?`,
...
\`\`\``,
        ...
        },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-2-03-ticket-line-ballot-problem",
            chapter: 5,
            section: "5.2",
            sectionTitle: "Martingale and Random Walk",
            title: "Ticket Line (Ballot Problem)",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `2n people in line — n with \\$5 bills and n with \\$10 bills — want to buy a \\$5 ticket. The seller starts with no change. What is the probability all people can be served without reordering?`,
...
\`\`\``,
        ...
        },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-2-04-coin-sequence-expected-tosses-for-n-heads-in-a-row",
            chapter: 5,
            section: "5.2",
            sectionTitle: "Martingale and Random Walk",
            title: "Coin Sequence: Expected Tosses for n Heads in a Row",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `What is the expected number of fair coin tosses to get n consecutive heads?`,
...
> **Key insight:** Start with the optimal policy for every state of the **final stage** (maximum information, minimum uncertainty) and work backward toward the initial stage.` },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-3-01-dp-dice-game-up-to-3-rolls",
            chapter: 5,
            section: "5.3",
            sectionTitle: "Dynamic Programming",
            title: "DP Dice Game (Up to 3 Rolls)",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `Roll a die up to 3 times. After each of the first two rolls, you can either keep the value or roll again (forfeiting the current value). On the third roll, you must keep the result. What is the expected value and optimal strategy?`,
            solution: `**Stage 3 (final):** No choice. E[payoff] = (1+2+3+4+5+6)/6 = **3.5**
...
**Game value: $14/3 ≈ $4.67. Optimal strategy: stop at first roll if ≥5; stop at second roll if ≥4; always accept third roll.**`,
        ...
        },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-3-02-world-series-betting",
            chapter: 5,
            section: "5.3",
            sectionTitle: "Dynamic Programming",
            ...
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `The Red Sox and Rockies play a best-of-7 series (first to 4 wins). You have \\$100 to bet on the Red Sox on a double-or-nothing basis, but can only bet on individual games. Devise a strategy so that if the Red Sox win the series you net exactly +\\$100, and if they lose you net exactly −\\$100.`,
...
> **Connection to options pricing:** This is structurally identical to pricing a European binary option using a binomial tree. The bet size at each state is the delta of a dynamic hedge.`,
        ...
        },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-3-03-dynamic-dice-game",
            chapter: 5,
            section: "5.3",
            sectionTitle: "Dynamic Programming",
            ...
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `A casino game: roll a die repeatedly. After each roll 1–5 you earn that amount and can choose to stop or continue. If a 6 appears at any time, you lose all accumulated winnings and the game ends. How much should you pay to play (risk-neutral)?`,
...
Maximum payoff: \\$19 (roll a 5 from state n=14: 14+5=19). Recursive calculation gives **f(0) = \\$6.15**.`,
        ...
        },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-3-04-dynamic-card-game",
            chapter: 5,
            section: "5.3",
            sectionTitle: "Dynamic Programming",
            ...
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `A dealer draws from a shuffled standard 52-card deck (26 red, 26 black) one card at a time (no replacement). Red = +\\$1, black = −\\$1. You choose when to stop. What is the optimal stopping strategy and expected payoff?`,
...
---`,
    ...
    },
    {
      ...
      problemCount: 2,
      blocks: [
        { kind: 'prose', markdown: `### Brownian Motion: Definition and Properties
...
- \`Z(t) = exp(λW(t) − ½λ²t)\` is a martingale (**exponential martingale**) for any constant λ` },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-4-01-a-correlation-of-brownian-motion-and-its-square",
            chapter: 5,
            section: "5.4",
            sectionTitle: "Brownian Motion and Stochastic Calculus",
            title: "A — Correlation of Brownian motion and its square",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `What is Corr(Bₜ, Bₜ²)?`,
...
**Corr(Bₜ, Bₜ²) = 0** despite the obvious nonlinear dependence (Bₜ² = |Bₜ|²). This demonstrates that zero correlation does not imply independence.`,
        ...
        { kind: 'prose', markdown: `**B — P(B₁ > 0 and B₂ < 0)**
...
**Geometric insight:** The joint distribution of (X, Y) = (B₁, B₂−B₁) divides the 2D plane into 8 equally-probable sectors separated by lines x=0, y=0, y=x, y=−x. The event {X>0, Y<−X} occupies exactly one sector.` },
        { kind: 'prose', markdown: `### Stopping Time / First Passage Time
...
Since Bₜ = ±1 with probability 1: \`E[Bₜ²] = 1²\`. Therefore: **E[T] = 1**.` },
        { kind: 'prose', markdown: `**B — First passage time to level x**
...
**Expected value: E[τₓ] = ∞** even though P(τₓ < ∞) = 1. The distribution has such a heavy tail that the mean diverges.` },
        { kind: 'prose', markdown: `**C — BM hitting boundaries with and without drift**
...
\`\`\`` },
        {
          kind: 'problem',
          problem: {
            id: "ch5-5-4-02-d-probability-of-ever-reaching-1-with-positive-drift",
            chapter: 5,
            section: "5.4",
            sectionTitle: "Brownian Motion and Stochastic Calculus",
            title: "D — Probability of ever reaching −1 with positive drift",
            difficulty: "hard",
            keyTechnique: "",
            tags: ["markov-chains","martingales","brownian-motion","ito","sde","dp"],
            setup: `dX = dt + dW, X(0) = 0. What is P(X ever reaches −1)?`,
...
Positive drift makes it unlikely (but not impossible) for the process to ever reach −1.`,
        ...
        { kind: 'prose', markdown: `### Itô's Lemma
...
> A generalized Wiener process \`dx = a(x,t)dt + b(x,t)dW(t)\` is a martingale **if and only if** the drift coefficient \`a(x,t) = 0\`.` },
        { kind: 'prose', markdown: `### Itô's Lemma Problems
...
**Martingale check:** The drift term \`(Bₜ/2√t)dt ≠ 0\` whenever Bₜ ≠ 0, which occurs with probability 1. Therefore **Z(t) = √t · Bₜ is NOT a martingale**.` },
        { kind: 'prose', markdown: `**B — Is W(t)³ a martingale?**
...
> **Note:** W(t)² − t IS a martingale: d(W²−t) = 2W dW + dt − dt = 2W dW (drift = 0). ✓` },
        { kind: 'prose', markdown: `*End of Chapter 5*` },
      ],
    },
  ],
};
